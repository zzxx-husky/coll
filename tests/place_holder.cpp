...
GTEST_TEST(PlaceHolder, Concat) {
  ...
  z.process(coll::Left::value, 1);
  EXPECT_EQ(v.size(), 1);
  EXPECT_EQ(v.back(), 1 + 1);

  z.process(coll::Right::value, 2);
  EXPECT_EQ(v.size(), 2);
  EXPECT_EQ(v.back(), 2 * 2);

  z.process(coll::Left::value, 3);
  EXPECT_EQ(v.size(), 3);
  EXPECT_EQ(v.back(), 3 + 1);

  z.process(coll::Right::value, 4);
  EXPECT_EQ(v.size(), 4);
...
GTEST_TEST(PlaceHolder, Concat2) {
  ...
  w.process(coll::Left::value, coll::Left::value, 1);
  EXPECT_EQ(v.size(), 1);
  EXPECT_EQ(v.back(), 1 + 1);

  w.process(coll::Left::value, coll::Right::value, 2);
  EXPECT_EQ(v.size(), 2);
  EXPECT_EQ(v.back(), 2 * 2);

  w.process(coll::Right::value, coll::Left::value, 3);
  EXPECT_EQ(v.size(), 3);
  EXPECT_EQ(v.back(), 3 - 3);

  w.process(coll::Right::value, coll::Right::value, 4);
  EXPECT_EQ(v.size(), 4);
